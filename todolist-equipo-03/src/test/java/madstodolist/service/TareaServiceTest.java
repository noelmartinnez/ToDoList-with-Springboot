...
public class TareaServiceTest {
    ...
    @Test
    public void testNuevaTareaUsuarioConFecha() throws ParseException {
        // GIVEN
        // Un usuario en la BD
        ...
        // WHEN
        // creamos una nueva tarea asociada al usuario,
        TareaData nuevaTarea = new TareaData();
        nuevaTarea.setUsuarioId(usuarioId);
        nuevaTarea.setTitulo("Práctica 1 de MADS");
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        Date fechaLimite = dateFormat.parse("2023-12-11");
        nuevaTarea.setFechaLimite(fechaLimite);

        TareaData tareaNueva = tareaService.nuevaTareaUsuario(nuevaTarea);
        ...
        assertThat(tareas).hasSize(3);
        assertThat(tareas).contains(tareaNueva);
        assertThat(tareaNueva.getFechaLimite()).isEqualTo(fechaLimite);
    }
    ...
    public void testModificarTarea() throws ParseException {
        ...
        tareaService.modificaTarea(tareaId, "Limpiar los cristales del coche", EstadoTarea.POR_HACER, fechaLimite);
    ...
    @Test
    public void testModificarTareaConFecha() throws ParseException {
        // GIVEN
        // Un usuario y una tarea en la BD
    ...
    public void testDestacarTarea() {
        ...
        // WHEN
        // cambiamos el estado destacado de la tarea correspondiente al identificador,

        TareaData tareaAntes = tareaService.findById(tareaId);
        tareaService.destacarTarea(tareaId);

        // THEN
        // al buscar por el identificador en la base de datos se devuelve la tarea modificada
        TareaData tareaDespues = tareaService.findById(tareaId);

        // Aseguramos que el estado destacado cambió
        assertThat(tareaDespues.isDestacada()).isNotEqualTo(tareaAntes.isDestacada());
    }


    @Test
    public void testAllTareasUsuarioConDestacadasPrimero() {
        // GIVEN
        ...
        // WHEN
        // Destacamos la tarea después de asignarla al usuario
        tareaService.destacarTarea(tareaId2);

        // Obtenemos todas las tareas del usuario
        List<TareaData> tareas = tareaService.allTareasUsuario(usuarioId);

        // THEN
        // Verificamos que las tareas destacadas, si hay alguna, estén primero
        assertThat(tareas).isNotEmpty();

        boolean tieneDestacada = tareas.stream().anyMatch(TareaData::isDestacada);

        if (tieneDestacada) {
            // Si hay tareas destacadas, la primera tarea debe ser destacada
            assertThat(tareas.get(0).isDestacada()).isTrue();
        }
    }
}
