...
public class TareaWebTest {
    ...
    public void getNuevaTareaDevuelveForm() throws Exception {
        ...
        this.mockMvc.perform(get(urlPeticion))
                .andExpect(content().string(containsString("Fecha Limite")))
                .andExpect((content().string(allOf(
    ...
    public void editarTareaActualizaLaTarea() throws Exception {
        ...
        this.mockMvc.perform(get(urlListado))
                ...
                .andExpect(content().string(not(containsString("29-12-2019"))));
    }

    @Test
    public void toggleDestacadaActualizaTarea() throws Exception {
        // GIVEN
        ...
        // WHEN, THEN
        // Realizamos una petición POST al endpoint para destacar/desdestacar una tarea

        String urlToggleDestacada = "/tareas/" + tareaId + "/destacada";

        this.mockMvc.perform(post(urlToggleDestacada))
                .andExpect(status().isOk());

        // Y si consultamos la lista de tareas
        // deberíamos ver que la tarea tiene el emoji de estrella (⭐)
    ...
    @Test
    public void CambiarEstadoTarea() throws Exception {
        // GIVEN
        // Un usuario con dos tareas en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long usuarioId = ids.get("usuarioId");
        ...
        String url = "/tarea/" + tareaLavarCocheId + "/estado/enProceso";
        String urlEditar = "/tareas/" + tareaLavarCocheId + "/editar";
        ...
        this.mockMvc.perform(get(urlEditar))
                .andExpect(content().string(containsString("EN_PROCESO")));
    }
    ...
    public void mostrarCalendario() throws Exception {
        ...
        // WHEN, THE
        // Realizamos una petición Get al endpoint para mostrar el calendario

        String urlCalendario = "/tareas/calendario";

        this.mockMvc.perform(get(urlCalendario))
                .andExpect(status().isOk())
    ...
    @Test
    public void verTarea() throws Exception {
        // GIVEN
        // Un usuario con dos tareas en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long usuarioId = ids.get("usuarioId");
    ...
    @Test
    public void verTareaconComentarios() throws Exception {
        // GIVEN
        // Un usuario con dos tareas en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long usuarioId = ids.get("usuarioId");
        Long tareaId = ids.get("tareaId");
        ComentarioData comentario = comentarioService.nuevoComentario(usuarioId,tareaId,"holaaa");
        ComentarioData comentario2= comentarioService.nuevoComentario(usuarioId,tareaId,"adioos");
        when(managerUserSession.usuarioLogeado()).thenReturn(usuarioId);
        ...
        this.mockMvc.perform(get(url))
                .andExpect((content().string(allOf(
                        containsString("Lavar coche"),
                        containsString("holaaa"),
                        containsString("adioos")
                ))));
    }

    @Test
    public void BorrarComentario() throws Exception {
        // GIVEN
        // Un usuario con dos tareas en la BD
        Map<String, Long> ids = addUsuarioTareasBD();
        Long usuarioId = ids.get("usuarioId");
        ...
        // se realiza la petición GET al listado de tareas del usuario,
        // el HTML devuelto contiene las descripciones de sus tareas.
        String urlborrar="/comentario/"+comentario.getId()+"/borrar";

        this.mockMvc.perform(delete(urlborrar)).andExpect(status().isOk());
    ...
    public void CrearComentario() throws Exception {
        ...
        // se realiza la petición GET al listado de tareas del usuario,
        // el HTML devuelto contiene las descripciones de sus tareas.
        String urlborrar="/tareas/"+tareaId+"/comentarios/nuevo";

        this.mockMvc.perform(post(urlborrar).param("texto","holaaa")).andExpect(status().is3xxRedirection());
